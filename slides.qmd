...
[demo/generate-spooky-folder.R](https://github.com/jadeynryan/fs-rladies-sl/blob/main/demo/generate-spooky-folder.R)
...
[demo/code-from-slides.R](https://github.com/jadeynryan/fs-rladies-sl/blob/main/demo/code-from-slides.R)  
...
::: callout-warning
Must pass vector of names to `dir_create`. Otherwise,
`dir_create(root, "R", "data/raw", "data/processed", "output", "reports", "images")` will result in this folder path: `scary-analysis/R/data/raw/data/processed/output/reports/images/`.
:::
...
See the [`make_clean_names()` docs](https://sfirke.github.io/janitor/reference/make_clean_names.html).
...
5. ⚠️**Review first before renaming!**⚠️
...
# Use functions to repeat this process for multiple folders {.section}
...
4. To use, type `project` in an R script or in the Console and then press `Shift+Tab`.

:::

## Cleanup {visibility="hidden"}

```{r}
#| echo: false

# Cleanup
dirs <- c("demo/scary-analysis", "demo/other-scary-analysis", "demo/spooky-folder", "demo/spooky_folder")
dir_delete(dirs)
```
...
-   [{fs} pkgdown website](https://fs.r-lib.org/index.html)
...
- [{stringr} regular expressions article](https://stringr.tidyverse.org/articles/regular-expressions.html)
:::
:::
